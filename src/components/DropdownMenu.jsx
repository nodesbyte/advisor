              ...
              <div className="flex flex-col md:flex-row gap-6">
                <div className="md:w-1/3 pr-6 border-r border-white flex flex-col justify-center">
                  ...
                  <Link
                    to={link}
                    onClick={handleClose}
                    className="mt-6 inline-flex items-center gap-2 px-5 py-3 w-fit rounded-full border border-white bg-[#6e4b3a] text-white text-sm hover:bg-[#5a3d2f] transition"
                  >
                    View More <ChevronRight size={18} />
                  </Link>
                </div>

                <div className="md:w-1/3 px-6 flex flex-col justify-center gap-2 border-r border-white">
                  {items.map((item, index) => (
                    <Link
                      key={index}
                      to={`/${plainTitle.toLowerCase()}/${item.toLowerCase().replace(/\s+/g, "-")}`}
                      onClick={handleClose}
                      className="block px-4 py-2 text-lg text-white hover:underline"
                    >
                      {item}
                    </Link>
                  ))}
