...
const TeamPage = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(teamCollection, (snapshot) => {
      ...
      // optional: sort by createdAt newest → oldest
      data.sort((a, b) => {
  ...
  return (
    <Box sx={{ maxWidth: "1000px", mx: "auto", mt: 8, px: 2, pb: 12 }}>
      ...
      <Box sx={{ display: "flex", justifyContent: "center" }}>
        <Tabs
          ...
          sx={{
            ...
            "& .MuiTabs-indicator": { backgroundColor: "#BC9A87" },
          }}
        >
          {teamMembers.map((member, index) => (
            <Tab key={member.id} label={member.name} />
          ))}
        </Tabs>
      </Box>
      ...
      <Box sx={{ mt: 4 }}>
        ...
        {/* Experience outside card */}
        {selectedMember.experience && (
          <Box sx={{ mt: 4 }}>{selectedMember.experience}</Box>
        )}
      </Box>
    </Box>
  );
